...
import { ArrowLeft, Send, MoreVertical, Users, Hash, Bell, Pin, UserPlus, Smile, Plus, Gift, Mic, Menu } from "lucide-react";
...
const ChatRoomSidebar = ({ chatId, navigate }: any) => {
        ...
        <div className="flex-1 overflow-y-auto p-2">
          ...
          {mockChats.filter(c => c.type !== "direct").map(c => (
            ...
            >
              ...
              {open && <span className="text-sm font-medium truncate">{c.title}</span>}
            </button>
          ))}
        </div>
      </SidebarContent>
  ...
  return (
    <SidebarProvider>
      <div className="min-h-screen bg-background flex w-full">
        {/* Left Sidebar - Collapsible Channel List */}
        <div className="hidden md:block">
          <ChatRoomSidebar chatId={chatId} navigate={navigate} />
        </div>
