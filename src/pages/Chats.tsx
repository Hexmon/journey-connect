...
import { Hash, Lock, User, Users, Plus, Search, ChevronDown, Settings, Volume2, UserPlus, Bell, Pin, MapPin, Compass, MessageCircle, Menu } from "lucide-react";
    ...
    <Sidebar className={open ? "w-60" : "w-14"} collapsible="icon">
      <SidebarContent className="flex flex-col h-full">
        ...
        <div className="h-12 px-4 flex items-center justify-between border-b border-border shadow-sm hover:bg-muted/50 cursor-pointer transition-colors">
          {open && <h1 className="font-bold text-sm">Community Hub</h1>}
          {open && <ChevronDown className="h-4 w-4 text-muted-foreground" />}
          {!open && <Hash className="h-5 w-5 text-muted-foreground" />}
        </div>
        ...
        {!open && (
          <div className="hidden md:flex flex-col gap-1 p-2 border-b border-border">
            <button onClick={() => navigate("/map")} className="p-2 rounded-md hover:bg-muted/80 transition-colors">
              <MapPin className="h-5 w-5 text-muted-foreground" />
            </button>
            <button onClick={() => navigate("/discover")} className="p-2 rounded-md hover:bg-muted/80 transition-colors">
              <Compass className="h-5 w-5 text-muted-foreground" />
            </button>
            <button className="p-2 rounded-md bg-primary/10">
              <MessageCircle className="h-5 w-5 text-primary" />
            </button>
            <button onClick={() => navigate("/forum")} className="p-2 rounded-md hover:bg-muted/80 transition-colors">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <Collapsible open={forumsOpen} onOpenChange={setForumsOpen}>
            <CollapsibleTrigger className="w-full flex items-center gap-0.5 px-2 py-1 hover:bg-transparent group">
              ...
              {!open && <Hash className="h-5 w-5 text-muted-foreground mx-auto" />}
            </CollapsibleTrigger>
        ...
        {/* User Bar at Bottom */}
        <div className={`${open ? "h-14 px-2" : "h-14 px-0"} flex items-center gap-2 bg-muted/30 border-t border-border`}>
          {open ? (
            <>
  ...
  return (
    <SidebarProvider>
      <div className="min-h-screen bg-background flex w-full">
        ...
        <ChatSidebar
          search={search}
          forumsOpen={forumsOpen}
          setForumsOpen={setForumsOpen}
